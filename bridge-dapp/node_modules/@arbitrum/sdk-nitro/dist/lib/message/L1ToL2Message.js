...
class L1ToL2MessageReader extends L1ToL2Message {
    ...
    async getSuccessfulRedeem() {
        ...
        if (creationReceipt) {
            const iFace = ArbRetryableTx__factory_1.ArbRetryableTx__factory.createInterface();
            ...
            while (fromBlock.number < maxBlock) {
                ...
                const redeemEvents = redeemEventLogs.map(r => iFace.parseLog(r).args);
                const successfulRedeem = (await Promise.all(redeemEvents.map(e => this.l2Provider.getTransactionReceipt(e.retryTxHash)))).filter(r => r.status === 1);
                ...
                if (toBlock.timestamp - creationBlock.timestamp >
...
class L1ToL2MessageWriter extends L1ToL2MessageReader {
    ...
    async redeem(overrides) {
        ...
        if (status === L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2) {
            ...
            throw new errors_1.ArbSdkError(`Cannot redeem. Message status: ${status} must be: ${L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2}.`);
        }
    }
    /**
    ...
    async cancel(overrides) {
        ...
        if (status === L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2) {
            ...
            throw new errors_1.ArbSdkError(`Cannot cancel. Message status: ${status} must be: ${L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2}.`);
        }
    }
}
