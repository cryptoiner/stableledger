...
class L2TransactionReceipt {
    ...
    getRedeemScheduledEvents() {
        ...
        const redeemScheduledEvents = this.logs.filter(l => l.topics[0] === redeemTopic);
        return redeemScheduledEvents.map(r => iFace.parseLog(r).args);
    }
    ...
    async getBatchNumber(l2Provider) {
        const nodeInterface = NodeInterface__factory_1.NodeInterface__factory.connect(constants_1.NODE_INTERFACE_ADDRESS, l2Provider);
        const rec = await (0, arbProvider_1.getArbTransactionReceipt)(l2Provider, this.transactionHash);
        if (rec == null)
