...
const L2CustomGateway__factory_1 = require("../abi/factories/L2CustomGateway__factory");
...
class Erc20Bridger extends assetBridger_1.AssetBridger {
    ...
    async approveToken(params) {
        ...
        const gatewayAddress = await this.getL1GatewayAddress(params.erc20L1Address, params.l1Signer.provider);
        const contract = await ERC20__factory_1.ERC20__factory.connect(params.erc20L1Address, params.l1Signer);
        return contract.functions.approve(gatewayAddress, params.amount || Erc20Bridger.MAX_APPROVAL, params.overrides || {});
    }
    /**
    ...
    async getL2WithdrawalEvents(l2Provider, gatewayAddress, filter, l1TokenAddress, fromAddress) {
        ...
        const events = (await eventFetcher.getEvents(gatewayAddress, L2ArbitrumGateway__factory_1.L2ArbitrumGateway__factory, contract => contract.filters.WithdrawalInitiated(null, fromAddress || null), filter)).map(a => (Object.assign({ txHash: a.transactionHash }, a.event)));
    ...
    async getDepositParams(params) {
        ...
        const l1GatewayAddress = await this.getL1GatewayAddress(erc20L1Address, l1Signer.provider);
        ...
        const baseFee = await (0, lib_1.getBaseFee)(l1Signer.provider);
        const estimates = await gasEstimator.estimateAll(l1GatewayAddress, l2Dest, depositCalldata, estimateGasCallValue, baseFee, sender, sender, l1Signer.provider, tokenGasOverrides);
        const data = abi_1.defaultAbiCoder.encode(['uint256', 'bytes'], [estimates.maxSubmissionFee, '0x']);
        return {
    ...
    async depositTxOrGas(params, estimate) {
        ...
        const l1GatewayRouter = L1GatewayRouter__factory_1.L1GatewayRouter__factory.connect(this.l2Network.tokenBridge.l1GatewayRouter, params.l1Signer);
        return await (estimate
            ? l1GatewayRouter.estimateGas
            : l1GatewayRouter.functions).outboundTransfer(depositParams.erc20L1Address, depositParams.destinationAddress, depositParams.amount, depositParams.l2GasLimit, depositParams.l2MaxFeePerGas, depositParams.data, Object.assign({ value: depositParams.depositCallValue }, params.overrides));
    }
    /**
    ...
    async withdrawTxOrGas(params, estimate) {
        ...
        const l2GatewayRouter = L2GatewayRouter__factory_1.L2GatewayRouter__factory.connect(this.l2Network.tokenBridge.l2GatewayRouter, params.l2Signer);
        return (estimate ? l2GatewayRouter.estimateGas : l2GatewayRouter.functions)['outboundTransfer(address,address,uint256,bytes)'](params.erc20l1Address, to, params.amount, '0x', Object.assign({}, (params.overrides || {})));
    }
    /**
...
class AdminErc20Bridger extends Erc20Bridger {
    ...
    async registerCustomToken(l1TokenAddress, l2TokenAddress, l1Signer, l2Provider) {
        ...
        const gasPriceEstimator = new L1ToL2MessageGasEstimator_1.L1ToL2MessageGasEstimator(l2Provider);
        ...
        const il2CustomGateway = L2CustomGateway__factory_1.L2CustomGateway__factory.createInterface();
        const l2SetTokenCallData = il2CustomGateway.encodeFunctionData('registerTokenFromL1', [[l1TokenAddress], [l2TokenAddress]]);
        const l1SignerAddr = await l1Signer.getAddress();
        const baseFee = await (0, lib_1.getBaseFee)(l1Signer.provider);
        const setTokenEstimates = await gasPriceEstimator.estimateAll(this.l2Network.tokenBridge.l1CustomGateway, this.l2Network.tokenBridge.l2CustomGateway, l2SetTokenCallData, constants_1.Zero, baseFee, l1SignerAddr, l1SignerAddr, l1Signer.provider);
        // 2. setGateway
        const iL2GatewayRouter = L2GatewayRouter__factory_1.L2GatewayRouter__factory.createInterface();
        const l2SetGatewaysCallData = iL2GatewayRouter.encodeFunctionData('setGateway', [[l1TokenAddress], [this.l2Network.tokenBridge.l1CustomGateway]]);
        const setGatwayEstimates = await gasPriceEstimator.estimateAll(this.l2Network.tokenBridge.l1GatewayRouter, this.l2Network.tokenBridge.l2GatewayRouter, l2SetGatewaysCallData, constants_1.Zero, baseFee, l1SignerAddr, l1SignerAddr, l1Signer.provider);
        // now execute the registration
        const customRegistrationTx = await l1Token.registerTokenOnL2(l2TokenAddress, setTokenEstimates.maxSubmissionFee, setGatwayEstimates.maxSubmissionFee, setTokenEstimates.gasLimit, setGatwayEstimates.gasLimit, setGatwayEstimates.maxFeePerGas, setTokenEstimates.totalL2GasCosts, setGatwayEstimates.totalL2GasCosts, l1SenderAddress, {
            value: setTokenEstimates.totalL2GasCosts.add(setGatwayEstimates.totalL2GasCosts),
        });
        return L1Transaction_1.L1TransactionReceipt.monkeyPatchWait(customRegistrationTx);
    }
    /**
    ...
    async getL1GatewaySetEvents(l1Provider, filter) {
        ...
        return (await eventFetcher.getEvents(l1GatewayRouterAddress, L1GatewayRouter__factory_1.L1GatewayRouter__factory, t => t.filters.GatewaySet(), filter)).map(a => a.event);
    }
    /**
    ...
    async getL2GatewaySetEvents(l2Provider, filter, customNetworkL2GatewayRouter) {
        ...
        const eventFetcher = new eventFetcher_1.EventFetcher(l2Provider);
        return (await eventFetcher.getEvents(l2GatewayRouterAddress, L1GatewayRouter__factory_1.L1GatewayRouter__factory, t => t.filters.GatewaySet(), filter)).map(a => a.event);
    }
    /**
     ...
     * @returns
     */
    async setGateways(l1Signer, l2Provider, tokenGateways) {
        if (!signerOrProvider_1.SignerProviderUtils.signerHasProvider(l1Signer)) {
            throw new errors_1.MissingProviderArbSdkError('l1Signer');
        }
        await this.checkL1Network(l1Signer);
        await this.checkL2Network(l2Provider);
        const estimator = new L1ToL2MessageGasEstimator_1.L1ToL2MessageGasEstimator(l2Provider);
        const baseFee = await (0, lib_1.getBaseFee)(l1Signer.provider);
        const iL2GatewayRouter = L2GatewayRouter__factory_1.L2GatewayRouter__factory.createInterface();
        ...
        const l1SignerAddr = await l1Signer.getAddress();
        const estimates = await estimator.estimateAll(this.l2Network.tokenBridge.l1GatewayRouter, this.l2Network.tokenBridge.l2GatewayRouter, l2SetGatewaysCallData, constants_1.Zero, baseFee, l1SignerAddr, l1SignerAddr, l1Signer.provider);
        ...
        const res = await l1GatewayRouter.functions.setGateways(tokenGateways.map(tG => tG.tokenAddr), tokenGateways.map(tG => tG.gatewayAddr), estimates.gasLimit, estimates.maxFeePerGas, estimates.maxSubmissionFee, { value: estimates.totalL2GasCosts });
