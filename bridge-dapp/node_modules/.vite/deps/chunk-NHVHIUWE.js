...
var HMAC = class extends Hash {
  constructor(hash9, _key) {
    ...
    pad4.set(key.length > blockLen ? hash9.create().update(key).digest() : key);
...
var hmac = (hash9, key, message) => new HMAC(hash9, key).update(message).digest();
hmac.create = (hash9, key) => new HMAC(hash9, key);
...
function _createCurveFields(type6, CURVE, curveOpts = {}, FpFnLE) {
  if (FpFnLE === void 0)
    FpFnLE = type6 === "edwards";
  if (!CURVE || typeof CURVE !== "object")
    throw new Error(`expected valid ${type6} CURVE object`);
  ...
  const _b = type6 === "weierstrass" ? "b" : "d";
...
function validateSigFormat(format9) {
  if (!["compact", "recovered", "der"].includes(format9))
...
function ecdh(Point2, ecdhOpts = {}) {
  ...
  return Object.freeze({ getPublicKey: getPublicKey4, getSharedSecret, keygen, Point: Point2, utils: utils2, lengths });
}
function ecdsa(Point2, hash9, ecdsaOpts = {}) {
  ahash(hash9);
  ...
  const hmac3 = ecdsaOpts.hmac || ((key, ...msgs) => hmac(hash9, key, concatBytes(...msgs)));
  const { Fp: Fp3, Fn } = Point2;
  const { ORDER: CURVE_ORDER, BITS: fnBits } = Fn;
  const { keygen, getPublicKey: getPublicKey4, getSharedSecret, utils: utils2, lengths } = ecdh(Point2, ecdsaOpts);
  ...
  function validateSigLength(bytes, format9) {
    validateSigFormat(format9);
    const size6 = lengths.signature;
    const sizer = format9 === "compact" ? size6 : format9 === "recovered" ? size6 + 1 : void 0;
    return _abytes2(bytes, sizer, `${format9} signature`);
  }
  class Signature {
    ...
    static fromBytes(bytes, format9 = defaultSigOpts_format) {
      validateSigLength(bytes, format9);
      let recid;
      if (format9 === "der") {
        const { r: r2, s: s2 } = DER.toSig(_abytes2(bytes));
        return new Signature(r2, s2);
      }
      if (format9 === "recovered") {
        recid = bytes[0];
        format9 = "compact";
    ...
    static fromHex(hex, format9) {
      return this.fromBytes(hexToBytes(hex), format9);
    }
    ...
    toBytes(format9 = defaultSigOpts_format) {
      validateSigFormat(format9);
      if (format9 === "der")
    ...
    toHex(format9) {
      return bytesToHex(this.toBytes(format9));
    }
    // TODO: remove
    assertValidity() {
    }
  ...
  function validateMsgAndHash(message, prehash) {
    ...
    return prehash ? _abytes2(hash9(message), void 0, "prehashed message") : message;
  }
  ...
  function sign7(message, secretKey, opts = {}) {
    message = ensureBytes("message", message);
    const { seed, k2sig } = prepSig(message, secretKey, opts);
    const drbg = createHmacDrbg(hash9.outputLen, Fn.BYTES, hmac3);
  ...
  function verify6(signature, message, publicKey, opts = {}) {
    const { lowS, prehash, format: format9 } = validateSigOpts(opts, defaultSigOpts);
    publicKey = ensureBytes("publicKey", publicKey);
    message = validateMsgAndHash(ensureBytes("message", message), prehash);
    if ("strict" in opts)
      throw new Error("options.strict was renamed to lowS");
    const sig = format9 === void 0 ? tryParsingSig(signature) : Signature.fromBytes(ensureBytes("sig", signature), format9);
    if (sig === false)
      return false;
    try {
...
function weierstrass(c) {
  const { CURVE, curveOpts, hash: hash9, ecdsaOpts } = _ecdsa_legacy_opts_to_new(c);
  const Point2 = weierstrassN(CURVE, curveOpts);
  const signs = ecdsa(Point2, hash9, ecdsaOpts);
  return _ecdsa_new_output_to_legacy(c, signs);
}

// node_modules/@noble/curves/esm/_shortw_utils.js
function createCurve(curveDef, defHash) {
  const create4 = (hash9) => weierstrass({ ...curveDef, hash: hash9 });
...
function hash_to_field(msg, count, options) {
  ...
  const { p, k, m, hash: hash9, expand, DST } = options;
  ...
  if (expand === "xmd") {
    ...
    prb = expand_message_xof(msg, DST, len_in_bytes, k, hash9);
...
function ripemd1603(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = ripemd1602(from(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
function sha2564(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = sha2563(from(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes(bytes);
}
...
function verify2(options) {
  const { challenge: challenge2, hash: hash9 = true, metadata, publicKey, signature } = options;
...
function isSolidityType(type6) {
  return type6 === "address" || type6 === "bool" || type6 === "function" || type6 === "string" || bytesRegex2.test(type6) || integerRegex2.test(type6);
}
...
function checksum2(address) {
  ...
  for (let i = 0; i < 40; i += 2) {
    if (hash9[i >> 1] >> 4 >= 8 && characters[i]) {
      characters[i] = characters[i].toUpperCase();
    }
    if ((hash9[i >> 1] & 15) >= 8 && characters[i + 1]) {
...
(function(encodePacked3) {
  function encode10(type6, value, isArray = false) {
    ...
    if (arrayMatch && Array.isArray(value)) {
      ...
      const data = [];
      for (let i = 0; i < value.length; i++) {
        data.push(encode10(childType, value[i], true));
      }
      if (data.length === 0)
        return "0x";
      return concat4(...data);
    ...
    if (args_.length > 0) {
      ...
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i) => {
        if (Array.isArray(args_[i]))
          return args_[i].map((_, j) => encode10(param, args_[i][j]));
        return typeof args_[i] !== "undefined" && args_[i] !== null ? encode10(param, args_[i]) : null;
      })) ?? [];
    }
  }
  const selector = (() => {
...
var BLAKE2b = class extends BLAKE2 {
  constructor(opts = {}) {
    ...
    if (personalization !== void 0) {
      personalization = toBytes6(personalization);
...
var BLAKE3 = class _BLAKE3 extends BLAKE2 {
  constructor(opts = {}, flags = 0) {
    ...
    if (key !== void 0) {
      ...
      const k = toBytes6(key).slice();
      abytes2(k, 32);
      this.IV = u32(k);
      swap32IfBE(this.IV);
      this.flags = flags | B3_Flags.KEYED_HASH;
    } else if (hasContext) {
      const ctx = toBytes6(context);
  ...
  function inner(node) {
    ...
    while (level.length > 1) {
      ...
      for (let i = 0; i < level.length; i += 2)
        level_.push(hash2(concat3(level[i], level[i + 1])));
      level = level_;
    }
...
function contains(bloom, input) {
  ...
  for (const i of [0, 2, 4]) {
    const bit = hash9[i + 1] + (hash9[i] << 8) & 2047;
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b, from30, to3) => bytesToNumberBE(b.slice(from30, to3));
...
function fromHex7(hex, group) {
  return fromBytes7(toBytes2(hex), group);
}
...
function namehash(name) {
  ...
  for (let i = labels.length - 1; i >= 0; i -= 1) {
    ...
    const hashed = hashFromEncodedLabel ? fromHex(hashFromEncodedLabel) : keccak2563(fromString(labels[i]), { as: "Bytes" });
...
var hmac2 = (hash9, key, message) => new HMAC2(hash9, key).update(message).digest();
hmac2.create = (hash9, key) => new HMAC2(hash9, key);
...
function verify5(options) {
  ...
  return secp256k1.verify(signature, from(payload), toBytes3(publicKey), ...hash9 ? [{ prehash: true, lowS: true }] : []);
}
...
function pbkdf2Init(hash9, _password, _salt, _opts) {
  ...
  const PRF = hmac2.create(hash9, password);
...
function pbkdf2(hash9, password, salt, opts) {
  const { c, dkLen, DK, PRF, PRFSalt } = pbkdf2Init(hash9, password, salt, opts);
  let prfW;
...
var InvalidInputError2 = class _InvalidInputError extends BaseError4 {
...
function encodeType(value) {
  ...
  for (const type6 of deps) {
    result += `${type6}(${(types2[type6] ?? []).map(({ name, type: t }) => `${t} ${name}`).join(",")})`;
  }
  return result;
}
...
function encodeField(properties) {
  ...
  if (type6.lastIndexOf("]") === type6.length - 1) {
    ...
    return [
      { type: "bytes32" },
      keccak2563(encode3(typeValuePairs.map(([t]) => t), typeValuePairs.map(([, v]) => v)))
    ];
  }
...
*/
//# sourceMappingURL=chunk-NHVHIUWE.js.map
