...
function normalize(fn) {
  afn(fn);
  return { encode: (from4) => from4, decode: (to) => fn(to) };
}
...
var radix2carry = (from4, to) => from4 + (to - gcd(from4, to));
var powers = (() => {
  let res = [];
  for (let i = 0; i < 40; i++)
    res.push(2 ** i);
...
function convertRadix2(data, from4, to, padding2) {
  ...
  if (radix2carry(from4, to) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from4} to=${to} carryBits=${radix2carry(from4, to)}`);
  }
  let carry = 0;
  let pos = 0;
  const max = powers[from4];
...
function encode2(...parameters) {
  const [abiError, args] = (() => {
    if (Array.isArray(parameters[0])) {
      ...
      return [fromAbi2(abi2, name), args3];
    }
...
*/
//# sourceMappingURL=chunk-U3FYDVIJ.js.map
