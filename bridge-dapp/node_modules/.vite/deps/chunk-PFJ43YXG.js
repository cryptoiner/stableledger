...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-KQ4RCLZO.js");
  const signature_ = (() => {
...
var EstimateGasExecutionError = class extends BaseError {
  constructor(cause, { account, docsPath: docsPath4, chain, data, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to: to2, value }) {
    var _a;
...
async function getWebSocketRpcClient(url, options = {}) {
  ...
  return getSocketRpcClient({
    async getSocket({ onClose, onError, onOpen, onResponse }) {
      const WebSocket = await import("./native-GAE2WWSZ.js").then((module) => module.WebSocket);
...
function ripemd1604(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
...
function parseErc8010Signature(signature) {
  if (!isErc8010Signature(signature))
    return { signature };
  const { authorization: authorization_ox, to: to2, ...rest } = SignatureErc8010_exports.unwrap(signature);
  return {
    authorization: {
...
async function sendUnsignedTransaction(client, args) {
  var _a, _b, _c;
  const { accessList, data, from: from14, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to: to2, value, ...rest } = args;
  const chainFormat = (_c = (_b = (_a = client.chain) == null ? void 0 : _a.formatters) == null ? void 0 : _b.transactionRequest) == null ? void 0 : _c.format;
  const format5 = chainFormat || formatTransactionRequest;
  const request = format5({
    // Pick out extra data that might exist on the chain's transaction request type.
...
function fromBlobs(parameters) {
  ...
  return to2 === "hex" ? bytesToHex(trimmedData) : trimmedData;
}
...
function sidecarsToVersionedHashes(parameters) {
  ...
  const to2 = parameters.to ?? (typeof sidecars[0].blob === "string" ? "hex" : "bytes");
  const hashes = [];
...
//# sourceMappingURL=chunk-PFJ43YXG.js.map
